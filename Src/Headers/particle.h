...
public:
  ...
  Particle(float r, glm::vec2 p, GLFWwindow *window, glm::vec3 col, float e,
    ...
    mass = (4.0 / 3) * 3.14 * radius * radius * radius *
           density; // rendered as disks, not spheres
    alpha = alph;
  ...
  void update(float deltaTime, GLFWwindow *window, bool velocity_colouring,
    ...
    velocity =
        glm::clamp(velocity, -glm::vec2(maxVelocity), glm::vec2(maxVelocity));
    ...
    modelMatrix = glm::translate(
        modelMatrix,
        glm::vec3(position, 0.0f)); // the vector to translate by must be
                                    // withing -1,1 to stay on screen
    modelMatrix = glm::scale(modelMatrix, 2.0f * radius * glm::vec3(1.0));
  }

  glm::mat4 getModel() const { return modelMatrix; }
...
private:
  ...
  void handleBoundaryCollision() {
    ...
    // very important to clamp the positions, else the bounces are not
    // completely elastic
  ...
  void VelocityColouring() {
    ...
    if (speed < Green_Speed)
      colour = glm::vec3(0.0f, 1.0f, -1.0f) * (speed / Green_Speed) +
               glm::vec3(0.0f, 0.0f, 1.0f);
    else if (speed < Red_Speed)
      colour = glm::vec3(1.0f, -1.0f, 0.0f) *
                   ((speed - Green_Speed) / Red_Speed - Green_Speed) +
               glm::vec3(0.0f, 1.0f, 0.0f);
    else
      colour = glm::vec3(0.0f, 1.0f, 1.0f) *
                   ((speed - Red_Speed) / White_Speed - Red_Speed) +
  ...
  void calculateDrag(glm::vec2 &a_drag) const {
    ...
    glm::vec2 drag_dir =
        speed > 0.001f ? -normalize(velocity) : glm::vec2(0.0f);
    a_drag = 0.5f * 0.47f * 3.14f * radius * radius * speed * speed *
             (1 / mass) * fluid_density *
             drag_dir; // 0.47 is drag coefficient of sphere
  }
};
...
void handleParticleCollisions(Particle &p1, Particle &p2) {
  ...
  if (distance <= p1.radius + p2.radius - 0.0001f) {
    ...
    float velocity_along_normal =
        glm::dot(relative_velocity, normal); // relative velocity along normal
...
void handleGravity(Particle &p1, Particle &p2) {
  glm::vec2 distance = p1.position - p2.position; // already the squared
                                                  // distance

  float softening = 10.0f;
  float r2 = dot(distance, distance) + softening * softening;
  if (r2 > 400.0f * 400.0f || r2 == 0.01f)
    return;
  glm::vec2 dir = (r2 == 0) ? glm::vec2(0.0f) : distance / sqrtf(r2);
