...
int setup(int width, int height, GLFWwindow *&window, int res,
          unsigned int &circleVAO, unsigned int &circleVBO);
static void circleGenerate(glm::vec2 center, int res, unsigned int &VAO,
                           unsigned int &VBO);
...
void instancedArraySetup(unsigned int &Model_Projections, unsigned int &Colors,
                         int maxParticles, unsigned int circleVAO) {
  // generating instanced arrays
  glBindVertexArray(circleVAO);

  glGenBuffers(1, &Model_Projections);
  glBindBuffer(GL_ARRAY_BUFFER, Model_Projections);
  glBufferData(GL_ARRAY_BUFFER, maxParticles * sizeof(glm::mat4), NULL,
               GL_DYNAMIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, maxParticles * sizeof(glm::vec3), NULL,
               GL_DYNAMIC_DRAW);
